...
function Home() {
  ...
  const fetchShowsForDate = async (date) => {
    try {
      setLoading(true);
      const showsRes = await api.get(`/shows?date=${date}`);
  ...
  return (
    <div className="container">
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem', borderBottom: '1px solid #ddd' }}>
        {[
          { id: 'movies', label: 'Now Showing' },
          { id: 'browse', label: 'Browse by Date' },
          ...
          <button
            ...
            style={{
              background: activeSection === tab.id ? '#e50914' : 'transparent',
      ...
      {activeSection === 'movies' && (
        <section style={{ marginBottom: '3rem' }}>
          <h2 style={{ marginBottom: '1.5rem' }}>Now Showing</h2>
            ...
            {movies.map(movie => (
              <div key={movie._id} className="card">
                ...
                <div className="card-content">
                  <h3 className="card-title">{movie.title || 'Unknown Movie'}</h3>
      ...
      {activeSection === 'browse' && (
        <section style={{ marginBottom: '3rem' }}>
          <h2 style={{ marginBottom: '1rem' }}>Browse Movies by Date</h2>
          
          {/* Date Selection */}
          <div style={{ marginBottom: '2rem' }}>
            <h3 style={{ marginBottom: '1rem', fontSize: '1.1rem' }}>Select Date:</h3>
          ...
          <h3 style={{ marginBottom: '1rem' }}>
            Movies on {formatDateLabel(selectedDate)} ({new Date(selectedDate).toLocaleDateString()})
          </h3>
          ...
          {loading ? (
            ...
            <div className="alert alert-warning">
              No shows available for {formatDateLabel(selectedDate)}.
            </div>
          ) : (
                ...
                <div key={movie._id} className="card">
                  <div style={{ display: 'flex', gap: '1rem', padding: '1rem' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div>
                        <h4 style={{ marginBottom: '0.75rem', fontSize: '1rem' }}>Available Shows:</h4>
                          ...
                          {shows.map(show => (
                            ...
                            >
                              ...
                              <div style={{ fontSize: '0.7rem', opacity: 0.9 }}>
                                {show.cinema?.name || 'Cinema'}
                              </div>
                              <div style={{ fontSize: '0.7rem', opacity: 0.9 }}>
                                ₹{show.pricing?.regular || 'N/A'}+
                              </div>
                            </Link>
                          ))}
                        </div>
      ...
      {activeSection === 'cinemas' && (
        <section>
          <h2 style={{ marginBottom: '1.5rem' }}>Cinemas Near You</h2>
              ...
              >
                <div className="card">
                  <div className="card-content">
                    ...
                    <p className="card-subtitle">
                      {cinema.location?.address}, {cinema.location?.city}
                    </p>
